	...
	return (
		<Field>
			...
			{selectedSpecialtyIds?.length > 0 && (
				<div className='flex flex-wrap gap-2 mb-0 bg-muted rounded-lg'>
					{selectedSpecialtyIds?.map((id) => (
						<Badge
							key={id}
							variant='default'
							className='px-2 py-0 text-sm text-white'
						>
							{getSpecialtyTitle(id)}
							<Button
								variant='link'
								onClick={() => onRemoveSpecialty(id)}
								className='text-primary-foreground hover:text-destructive'
							>
								<X className='h-3 w-3' />
							</Button>
						</Badge>
