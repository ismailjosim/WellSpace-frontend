      ...
      <div className="container mx-auto text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {topDoctors.map((doctor) => (
            <Card
              key={doctor.id}
              className="group relative rounded-2xl border bg-card shadow-lg hover:shadow-xl transition-all duration-300"
            >
              <CardContent className="flex flex-col items-center text-center p-6 space-y-4">
                {/* Avatar */}
                <div className="w-20 h-20 flex items-center justify-center rounded-full bg-linear-to-br from-primary/10 to-secondary/20 border-2 border-primary/30 text-foreground font-semibold">
                  {doctor.initials}
                </div>
